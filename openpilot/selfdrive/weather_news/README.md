...
Voice is **Piper** (`en_US-lessac-high`) — neural, on-device, same woman and speed for Nice and Unhinged. Binary + model download once into `/data/weather_news` (~130 MB). Piper is `nice(10)` so it yields to openpilot. espeak-ng is fallback only if Piper cannot run.
