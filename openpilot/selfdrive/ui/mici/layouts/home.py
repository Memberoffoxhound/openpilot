...
class NetworkIcon(Widget):
  ...
  def _render(self, _):
    if self._net_type == NetworkType.wifi:
      draw_net_txt = {0: self._wifi_none_txt, 2: self._wifi_low_txt, 3: self._wifi_medium_txt,
                      4: self._wifi_full_txt, 5: self._wifi_full_txt}.get(self._net_strength, self._wifi_low_txt)
    elif self._net_type in (NetworkType.cell2G, NetworkType.cell3G, NetworkType.cell4G, NetworkType.cell5G):
      draw_net_txt = {0: self._cell_none_txt, 2: self._cell_low_txt, 3: self._cell_medium_txt,
                      4: self._cell_high_txt, 5: self._cell_full_txt}.get(self._net_strength, self._cell_none_txt)
    else:
...
class MiciHomeLayout(Widget):
  ...
  def _fmt_trip(self, meters: float, eng_m: float) -> str:
    ...
    txt = f"{dist:.1f}" if dist < 10 else f"{int(round(dist))}"
    return f"{txt}{unit} {pct}%"
