...
# angle override
STEER_OVERRIDE_MIN_TORQUE = 0.5  # Nm - typical steering bias + noise (deadzone)
STEER_OVERRIDE_MAX_TORQUE = 2.5  # Nm - typical torque before EPS hands_on_level=3
STEER_OVERRIDE_TORQUE_RANGE = STEER_OVERRIDE_MAX_TORQUE - STEER_OVERRIDE_MIN_TORQUE

STEER_OVERRIDE_MAX_LAT_ACCEL = 2.0  # m/s^2 - similar to Tesla comfort steering
...
CoopSteeringDataSP = namedtuple("CoopSteeringDataSP", ["steeringAngleDeg", "lat_active"])
...
class CoopSteeringCarController:
  ...
  def update_override_angle(self, apply_angle_delta: float, driver_torque: float, vEgo: float, VM: VehicleModel) -> float:
  ...
  def update(self, apply_angle, lat_active, CP, CS, VM: VehicleModel) -> CoopSteeringDataSP:
    angle_coop_enabled = bool(CP.flags & TeslaFlags.COOP_STEERING.value)
